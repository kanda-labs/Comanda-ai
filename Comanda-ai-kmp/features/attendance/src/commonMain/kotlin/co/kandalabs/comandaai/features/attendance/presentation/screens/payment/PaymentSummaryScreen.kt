        ...
        val actions: (PaymentSummaryAction) -> Unit = { action ->
            when (action) {
                ...
                is PaymentSummaryAction.SHOW_FINISH_PAYMENT_CONFIRMATION -> {
                    viewModel.showFinishPaymentConfirmation()
                }

                is PaymentSummaryAction.HIDE_FINISH_PAYMENT_CONFIRMATION -> {
                    viewModel.hideFinishPaymentConfirmation()
                }
...
private fun PaymentSummaryScreenContent(
    ...
    Surface(
        ...
        // Finish Payment Confirmation Modal
        if (state.showFinishPaymentConfirmation) {
            FinishPaymentConfirmationModal(
                isVisible = state.showFinishPaymentConfirmation,
                state = state,
                onDismiss = { action(PaymentSummaryAction.HIDE_FINISH_PAYMENT_CONFIRMATION) },
                onConfirm = { action(PaymentSummaryAction.FINISH_PAYMENT) }
            )
        }
    }
...
private fun PaymentSummaryBottom(
    ...
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .background(ComandaAiTheme.colorScheme.surface)
    ) {
        Column(
